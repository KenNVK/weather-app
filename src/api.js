...
const getWeatherData = async (lon, lat) => {
  const weather = await axios.get(
    `https://api.openweathermap.org/data/2.5/onecall?&lon=${lon}&lat=${lat}&exclude=hourly,minutely,alerts&units=metric&appid=${owmApiKey}`
  );
  const location = await axios.get(
    `https://api.mapbox.com/geocoding/v5/mapbox.places/${lon},${lat}.json?access_token=${mapboxApiKey}&types=country,place&limit=1`
  );
